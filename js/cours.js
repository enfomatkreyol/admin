...
function editcours() {
    document.getElementById('home').style.display = 'none';
   document.querySelector('.editoreArticles').style.display = 'none';
   document.querySelector('.editoreCours').style.display = 'block';
}
...
function subcategory() {
    ...
    const subcategorySelect = document.getElementById('ssubcategory');
    const p = document.getElementById('showcategorie');

    // Clear previous options
    subcategorySelect.innerHTML = '';

    // Assuming 'database' is defined somewhere else in your code
    // Replace 'database.ref(selectedCategory)' with appropriate reference based on your Firebase setup
    database.ref('COURS/' + selectedCategory).once('value')
        .then(snapshot => {
            const categoryData = snapshot.val();
            ...
            if (categoryData) {
                ...
                // Hide the first 4 options
                for (let i = 0; i < Math.min(count, 4); i++) {
                    subcategorySelect.options[i].hidden = true;
                }
            } else {
                alert('No data found for the selected category.');
            }
        ...
        .catch(error => {
            ...
            alert('An error occurred while retrieving data from Firebase.');
        });
}
...
function ssubcategory() {
    // You can get the selected category if needed
    const sselectedCategory = document.getElementById('subcategory').value;
  ...
  function updateTitle() {
    ...
    const sselectedSubcategory = document.getElementById('ssubcategory').value;

    // Update the title input value with the selected subcategory
    document.getElementById('title').value = sselectedSubcategory;
  }
...
function save() {
    ...
    database.ref(category +"/"+ subcategory +"/"+ title).set(dataToSave);
...
function search() {
    ...
    //var imglink = document.getElementById('imagelink').value;
    console.log(title);
    database.ref(category +"/"+ subcategory +"/"+ title).once('value', (snapshot) => {
...
function update() {
  ...
  database.ref(category +"/"+ subcategory +"/"+ title).update(dataToUpdate);
